...
--I (Fernando) changed this function so the table is created as a list of decreasing scores:
function h.set(filename, places, name, score)
	...
	if not h.load() then
		h.scores = {}
		for i = 1, places do
			h.scores[i] = {math.ceil(score - (score / 16) * (i - 1)), name} --Changed here.
		end
	end
end
...
function h.add(name, score)
  ...
  --I (Fernando) added this so the table doesn't grows:
  table.remove(h.scores, #h.scores)
end
